...
public:
        ...
        executor(executor)
    {
        ...
        // stream.reset(new Stream(executor));
        // stream->Init();
        // nccl_stream = ((CUDAStream*)stream->implementation())->cuda_stream();
    ...
    void AddEntry(int op_num, const Tensor* data, DoneCallback done_callback, CUevent op_event)
    {
        ...
        pending_comm.push_front(new CommEntry({
            ...
            data->dtype() == DT_HALF ? ncclHalf : ncclFloat,
    ...
    void NcclCommThread()
    {
        ...
        while (true)
        {
            ...
            if (prereduce)
            {
                ...
                nccl_result = ncclReduce(entry->buffer, entry->buffer, entry->count, entry->dtype, ncclSum, 0, comm_local, nccl_stream);
                if (nccl_result == ncclSuccess)
                {
                    // reduce among all machines with the root gpu on each
                    if (local_rank == 0 && global_size > 1)
                        nccl_result = ncclAllReduce(entry->buffer, entry->buffer, entry->count, entry->dtype, ncclSum, comm_global, nccl_stream);
                    // broadcast root gpu results out to the rest of the gpus on each machine
                    if (nccl_result == ncclSuccess)
                        nccl_result = ncclBroadcast(entry->buffer, entry->buffer, entry->count, entry->dtype, 0, comm_local, nccl_stream);
                }
            }
            else
            {
...
public:
    ...
    void AddEntry(int op_num, int comm_idx, const Tensor* data, DoneCallback done_callback, CUevent op_event)
    {
        comms[comm_idx]->AddEntry(op_num, data, std::move(done_callback), op_event);
    }
};
...
private:
    int op_num, num_comms, comm_idx, sync_size, prereduce;
    CUevent op_event;
    std::string logfile;
...
public:
    ...
    {
        OP_REQUIRES_OK(ctx, ctx->GetAttr("op_num",    &op_num   ));
    ...
    void ComputeAsync(OpKernelContext* ctx, DoneCallback done) override
    {
        ...
        // Tensor* output;
        // OP_REQUIRES_OK_ASYNC(ctx, ctx->allocate_output(0, input.shape(), &output), done);

        auto actual_done = [ctx, done](Status s) {
        ...
        CUstream cu_stream = ((CUDAStream*)stream->implementation())->cuda_stream();
        ...
        NcclManager::instance(num_comms, prereduce, executor)->
            AddEntry(op_num, comm_idx, &input, std::move(actual_done), op_event);
    }
